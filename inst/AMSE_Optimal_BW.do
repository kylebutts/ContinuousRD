//This program estimates the AMSE optimal bandwidths for tau_u and pi//
...
//Outcomes: 1. Log(Assets), 2. log(Assets/CapitalSurplus), and 3. Suspension//
	gen Y=log(Assets)
...
// For the adjustment term in Vpim, the upper bounds of the support of T1 and T0//
	global Tubp = r(max)
	global Tubm = r(max)
// For the adjustment term in Vpim, the lower bound of the support of T1 and T0// 
	global Tlbp = log(50000) 
	global Tlbm = log(25000)	
...
//Estimate Bias of the estimated tau_u and optimal bandwidth for estimating tau_u//
	...
	mat `opt_h_Rtau'[`r',1] = (`Vtau'[`r',1]/(8*$n) )^(1/6) *( `biasTTau_u'/`biasRTau_u'^5 )^(1/12)	
	mat `opt_h_Ttau'[`r',1] = (`Vtau'[`r',1]/(8*$n) )^(1/6) *( `biasRTau_u'/`biasTTau_u'^5 )^(1/12)	
...
//Estimate bias of pi//
	...
	svmat `biasRPi_u'
	egen `biasRPi'=total(`biasRPi_u'1*abs(qte1)/`w_sum')
